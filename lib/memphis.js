...
class Memphis {
    ...
    async station({ name, retentionType = retentionTypes.MAX_MESSAGE_AGE_SECONDS, retentionValue = 604800, storageType = storageTypes.DISK, replicas = 1, idempotencyWindowMs = 120000, schemaName = '', sendPoisonMsgToDls = true, sendSchemaFailedMsgToDls = true, tieredStorageEnabled = false, partitionsNumber = 1, }) {
        var _a;
        try {
            ...
            const res = await this.brokerManager.request('$memphis_station_creations', data, { timeout: 20000 });
    ...
    async enforceSchema({ name, stationName }) {
        try {
            if (!this.isConnectionActive)
                throw new Error('Connection is dead');
            if (name === '' || stationName === '') {
            ...
            const res = await this.brokerManager.request('$memphis_schema_attachments', data, { timeout: 20000 });
            const errMsg = res.data.toString();
            if (errMsg != '') {
                throw (0, utils_1.MemphisError)(new Error(errMsg));
            }
    ...
    async detachSchema({ stationName }) {
        try {
            ...
            let errMsg = await this.brokerManager.request('$memphis_schema_detachments', data, { timeout: 20000 });
    ...
    async producer({ stationName, producerName, genUniqueSuffix = false }) {
        ...
        try {
            ...
            let createRes = await this.brokerManager.request('$memphis_producer_creations', data, { timeout: 20000 });
    ...
    async consumer({ stationName, consumerName, consumerGroup = '', pullIntervalMs = 1000, batchSize = 10, batchMaxTimeToWaitMs = 5000, maxAckTimeMs = 30000, maxMsgDeliveries = 10, genUniqueSuffix = false, startConsumeFromSequence = 1, lastMessages = -1, consumerPartitionKey = null, }) {
        var _a;
        try {
            if (!this.isConnectionActive)
                throw new Error('Connection is dead');
            ...
            let createRes = await this.brokerManager.request('$memphis_consumer_creations', data, { timeout: 20000 });
            ...
            this.stationPartitions.set(internal_station, partitions);
            const consumer = new consumer_1.Consumer(this, stationName, consumerName, consumerGroup, pullIntervalMs, batchSize, batchMaxTimeToWaitMs, maxAckTimeMs, maxMsgDeliveries, startConsumeFromSequence, lastMessages, realName, partitions, consumerPartitionKey);
    ...
    async produce({ stationName, producerName, genUniqueSuffix = false, message, ackWaitSec, asyncProduce, headers, msgId, producerPartitionKey = null }) {
        let producer;
        if (!this.isConnectionActive)
    ...
    async fetchMessages({ stationName, consumerName, consumerGroup = '', genUniqueSuffix = false, batchSize = 10, maxAckTimeMs = 30000, batchMaxTimeToWaitMs = 5000, maxMsgDeliveries = 10, startConsumeFromSequence = 1, lastMessages = -1, consumerPartitionKey = null, }) {
        let consumer;
        if (!this.isConnectionActive)
    ...
    async createSchema({ schemaName, schemaType, schemaFilePath }) {
        try {
            ...
            let createRes = await this.brokerManager.request('$memphis_schema_creations', data, { timeout: 20000 });
            createRes = this.JSONC.decode(createRes.data);
            if (createRes.error != "")
                throw (0, utils_1.MemphisError)(new Error(createRes.error));
        }
    ...
    _getPartitionFromKey(key, stationName) {
        return new Promise((resolve, reject) => {
            ...
            mmh3.murmur32(key, seed, (err, hashValue) => {
                if (err) {
                    reject(err);
                    return;
                }
