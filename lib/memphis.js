...
class Memphis {
    ...
    async producer({ stationName, producerName, genUniqueSuffix = false }) {
        try {
            ...
            const producer = new producer_1.Producer(this, producerName, stationName, realName);
    ...
    async consumer({ stationName, consumerName, consumerGroup = '', pullIntervalMs = 1000, batchSize = 10, batchMaxTimeToWaitMs = 5000, maxAckTimeMs = 30000, maxMsgDeliveries = 10, genUniqueSuffix = false, startConsumeFromSequence = 1, lastMessages = -1 }) {
        try {
            ...
            const res = await this.brokerManager.request('$memphis_consumer_creations', data, { timeout: 10000 });
            const errMsg = res.data.toString();
            if (errMsg != '') {
                throw (0, utils_1.MemphisError)(new Error(errMsg));
            }
            const consumer = new consumer_1.Consumer(this, stationName, consumerName, consumerGroup, pullIntervalMs, batchSize, batchMaxTimeToWaitMs, maxAckTimeMs, maxMsgDeliveries, startConsumeFromSequence, lastMessages, realName);
