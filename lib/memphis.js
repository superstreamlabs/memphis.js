...
class Memphis {
    ...
    connect({ host, port = 6666, username, accountId = 1, connectionToken = '', password = '', reconnect = true, maxReconnect = 3, reconnectIntervalMs = 5000, timeoutMs = 2000, keyFile = '', certFile = '', caFile = '', suppressLogs = false }) {
    ...
    async producer({ stationName, producerName, genUniqueSuffix = false }) {
        try {
            ...
            producerName = genUniqueSuffix
                ? (0, utils_1.generateNameSuffix)(`${producerName}_`)
                : producerName;
            ...
            const producer = new producer_1.Producer(this, producerName, stationName, realName, createRes.partitions_update.partitions_list);
    ...
    async consumer({ stationName, consumerName, consumerGroup = '', pullIntervalMs = 1000, batchSize = 10, batchMaxTimeToWaitMs = 5000, maxAckTimeMs = 30000, maxMsgDeliveries = 10, genUniqueSuffix = false, startConsumeFromSequence = 1, lastMessages = -1 }) {
        try {
            ...
            const consumer = new consumer_1.Consumer(this, stationName, consumerName, consumerGroup, pullIntervalMs, batchSize, batchMaxTimeToWaitMs, maxAckTimeMs, maxMsgDeliveries, startConsumeFromSequence, lastMessages, realName, partitions);
    ...
    async fetchMessages({ stationName, consumerName, consumerGroup = '', genUniqueSuffix = false, batchSize = 10, maxAckTimeMs = 30000, batchMaxTimeToWaitMs = 5000, maxMsgDeliveries = 10, startConsumeFromSequence = 1, lastMessages = -1 }) {
        let consumer;
        if (!this.isConnectionActive)
