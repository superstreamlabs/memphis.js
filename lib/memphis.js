...
exports.memphis = exports.MemphisService = exports.createMemphisInstance = exports.RoundRobinProducerConsumerGenerator = void 0;
...
class Memphis {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                (async () => {
                    ...
                    var _d, _e;
                    try {
                        for (var _f = true, _g = __asyncValues(this.brokerManager.status()), _h; _h = await _g.next(), _a = _h.done, !_a;) {
                            _c = _h.value;
                            _f = false;
                            try {
                                const s = _c;
                                switch (s.type) {
                                    case 'update':
                                        ...
                                        break;
                                    case 'reconnecting':
                                        this.log(`trying to reconnect to memphis - ${(0, utils_1.MemphisErrorString)((_d = s.data) === null || _d === void 0 ? void 0 : _d.toString())}`);
                                        break;
                                    case 'disconnect':
                                        this.log(`disconnected from memphis - ${(0, utils_1.MemphisErrorString)((_e = s.data) === null || _e === void 0 ? void 0 : _e.toString())}`);
                                        this.isConnectionActive = false;
                                        break;
                                    case 'error':
                                        let err = s.data;
                                        if (err.toString().includes("AUTHORIZATION_VIOLATION")) {
                                            ...
                                            this.log((0, utils_1.MemphisErrorString)(err.toString()));
                                        }
                                        this.isConnectionActive = false;
    ...
    async _publish(fullSubjectName, message, publishOptions) {
    ...
    async consumer({ stationName, consumerName, consumerGroup = '', pullIntervalMs = 1000, batchSize = 10, batchMaxTimeToWaitMs = 1000, maxAckTimeMs = 30000, maxMsgDeliveries = 2, genUniqueSuffix = false, startConsumeFromSequence = 1, lastMessages = -1, consumerPartitionKey = null, consumerPartitionNumber = -1, timeoutRetry = 5 }) {
        var _a;
        try {
            if (!this.isConnectionActive)
                throw new Error('Connection is dead');
    ...
    async fetchMessages({ stationName, consumerName, consumerGroup = '', genUniqueSuffix = false, batchSize = 10, maxAckTimeMs = 30000, batchMaxTimeToWaitMs = 1000, maxMsgDeliveries = 2, startConsumeFromSequence = 1, lastMessages = -1, consumerPartitionKey = null, consumerPartitionNumber = -1, }) {
        let consumer;
        if (!this.isConnectionActive)
...
const createMemphisInstance = () => {
    return new Memphis();
};
exports.createMemphisInstance = createMemphisInstance;
