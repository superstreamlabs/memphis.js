...
class Memphis {
    ...
    connect({ host, port = 6666, username, connectionToken = '', password = '', reconnect = true, maxReconnect = 3, reconnectIntervalMs = 5000, timeoutMs = 15000, keyFile = '', certFile = '', caFile = '' }) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (this.connectionToken != '' && this.password != '') {
                    return reject((0, utils_1.MemphisError)(new Error(`Can't connect with connection token and password - must choose one method`)));
                }
                if (this.connectionToken == '' && this.password == '') {
                    return reject((0, utils_1.MemphisError)(new Error('Must connect with connection token or password - must choose one method')));
                }
